...
import os
import subprocess
from clustering_problems.min_height_tree_module import set_branch_node_features
...
def run_loandra_and_parse_results(loandra_path, execution_path):
    ...
    if not os.path.isfile(loandra_executable):
        raise FileNotFoundError(f"Loandra executable not found at {loandra_executable}")
    # Construct the full path to the CNF file
    full_execution_path = os.path.abspath(execution_path)
    #print(f"loandra_executable : {loandra_executable}")
    #print(f"full_execution_path : {full_execution_path}")
